...
var ProxySandbox = /*#__PURE__*/function () {
  function ProxySandbox(name) {
      ...
      get: function get(target, p) {
        ...
        if (p === 'document' || p === 'eval') {
          ...
          switch (p) {
            case 'document':
              // @ts-ignore
              if (document.getElementById('subapp-viewport').firstChild) {
                // @ts-ignore
                var a = document.getElementById('subapp-viewport').firstChild.children;
                var doc;
                ...
                console.log(doc); // @ts-ignore

                doc.body = doc;
                document.addEventListener = doc.addEventListener;
                return document;
              }
    ...
    this.proxy = proxy;
    var proxyDoc = new Proxy(document, {
      get: function get(target, p) {
        // @ts-ignore
        if ((p === 'addEventListener' || p === 'body') && document.getElementById('subapp-viewport').firstChild && document.getElementById('subapp-viewport').firstChild.shadowRoot) {
          // @ts-ignore
          var a = document.getElementById('subapp-viewport').firstChild.shadowRoot.children;
          var doc;
