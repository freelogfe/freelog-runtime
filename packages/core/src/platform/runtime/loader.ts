...
function getAppWrapperGetter(
  ...
  return () => {
    if (useLegacyRender) {
      if (strictStyleIsolation)
        throw new Error(
          "[freelog]: strictStyleIsolation can not be used with legacy render!"
        );
      if (scopedCSS)
        throw new Error(
          "[freelog]: experimentalStyleIsolation can not be used with legacy render!"
        );

      const appWrapper = document.getElementById.bind(document)(
        getWrapperId(appInstanceId)
      );
      assertElementExist(
        appWrapper,
...
function getLifecyclesFromExports(
  ...
  throw new Error(
    `[freelog] You need to export lifecycle functions in ${appName} entry`
  );
}
