  ...
  function ProxySandbox(name) {
    ...
    var proxy = new Proxy(fakeWindow, {
      set: function set(target, p, value) {
        if (_this2.sandboxRunning) {
          _this2.registerRunningApp(name, proxy); // @ts-ignore


          var hook = globalContext.proxyHooks.setHooks.get(p);
      ...
      get: function get(target, p) {
        _this2.registerRunningApp(name, proxy);
        ...
        var hook = globalContext.proxyHooks.getHooks.get(p);
    ...
    this.proxy = proxy; // @ts-ignore

    globalContext.saveSandBox && globalContext.saveSandBox(name, this);
    activeSandboxCount++;
  }
    ...
    value: function inactive() {
      ...
      if (--activeSandboxCount === 0) {
        variableWhiteList.forEach(function (p) {
          if (_this3.proxy.hasOwnProperty(p)) {
            // @ts-ignore
            delete _this3.globalContext[p];
          }
        });
      }
